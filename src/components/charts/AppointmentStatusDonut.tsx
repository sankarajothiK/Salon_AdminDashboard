...
const STATUS_COLORS: Record<string, string> = {
  COMPLETED: '#059669', // Emerald Primary
  SCHEDULED: '#34d399', // Mint
  CONFIRMED: '#047857', // Forest Emerald
  'IN PROGRESS': '#10b981', // Bright Emerald
...
export const AppointmentStatusDonut: React.FC<AppointmentStatusDonutProps> = ({ data }) => {
  ...
  if (!activeData.length) {
    return <div className="h-64 flex items-center justify-center text-xs text-emerald-800 font-alata font-bold">No appointments recorded</div>;
  }
          ...
          >
            {activeData.map((entry) => (
              <Cell
                ...
                fill={STATUS_COLORS[entry.status] || '#059669'}
                stroke="#ffffff"
                strokeWidth={2}
              />
            ))}
