  ...
  return (
    <div className="relative font-alata" ref={dropdownRef}>
      <button
        ...
        className={clsx(
          'flex items-center gap-1.5 sm:gap-2.5 px-2.5 sm:px-3.5 py-1.5 rounded-xl bg-white hover:bg-emerald-50 border-2 border-emerald-200 text-xs font-bold text-black transition-all shadow-2xs max-w-[150px] sm:max-w-[240px]'
        )}
      >
        {selectedSalonId === 'all' ? (
          <Globe className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-600 flex-shrink-0" />
        ) : (
          <div
            className="w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full border border-emerald-400 flex-shrink-0"
        ...
        <ChevronDown className="w-3.5 h-3.5 text-emerald-700 ml-0.5 flex-shrink-0" />
      </button>
