...
export const CustomersPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 font-alata">
      ...
      {/* Customers Table with Bold Black Typography */}
                  ...
                  {customers.map((c) => {
                    const getSegmentBadge = () => {
                      if (c.starred || c.segment === 'vip') {
                        return <span className="px-2.5 py-0.5 rounded-full text-[10.5px] font-bold bg-[#FFEBB8] text-black border border-[#BD5579]/40">VIP ⭐</span>;
                      }
                      if (c.segment === 'returning') {
                        return <span className="px-2.5 py-0.5 rounded-full text-[10.5px] font-bold bg-emerald-100 text-black border border-emerald-300">Returning</span>;
                      }
                      return <span className="px-2.5 py-0.5 rounded-full text-[10.5px] font-bold bg-blue-100 text-black border border-blue-300">New</span>;
                    };

                    return (
                      <tr key={c.id} className="hover:bg-[#fcf2f6]/60 transition-colors group">
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-xl bg-[#601D49] text-[#FFEBB8] border border-[#FFEBB8]/50 flex items-center justify-center font-bold text-xs flex-shrink-0 shadow-2xs">
                            ...
                            <div>
                              ...
                              {c.notes && <span className="text-[10.5px] text-black font-semibold block truncate max-w-[150px]">{c.notes}</span>}
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-4">
                          <span className="px-2.5 py-1 rounded-lg bg-[#fdf5e8] text-black border border-[#FFEBB8] text-[11px] font-bold">
                            {c.salon?.name || 'Salon'}
                          </span>
                        </td>
                        ...
                        <td className="px-5 py-4 text-right">
                          <Link
                            to={`/customers/${c.id}`}
                            className="px-3 py-1.5 text-xs rounded-xl bg-[#601D49] text-[#FFEBB8] hover:bg-[#7d265f] font-bold transition-all shadow-plum-sm"
                          >
                            Profile 360&deg;
                          </Link>
                        </td>
