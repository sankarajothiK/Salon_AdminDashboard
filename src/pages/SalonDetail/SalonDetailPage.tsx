...
export const SalonDetailPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 font-alata text-black">
      ...
      {/* KPI Cards Row with Bold Black text */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <div className="bg-white border border-[#BD5579]/20 rounded-2xl p-4 shadow-2xs">
      ...
      {/* Navigation Tabs */}
      <div className="flex border-b border-[#BD5579]/20 space-x-2">
        {[
          ...
          <button
            ...
            className={`px-4 py-3 text-xs font-bold border-b-2 transition-all ${
              activeTab === tab.key
                ? 'border-[#601D49] text-black bg-[#FFEBB8]/50 rounded-t-xl'
              ...
              <div className="space-y-2">
                {appointments.slice(0, 5).map((appt) => {
                  ...
                  return (
                    <div
                      key={appt.id}
                      className="p-3.5 rounded-xl bg-[#fdfafb] border border-[#BD5579]/15 flex items-center justify-between text-xs"
                    >
                      <div className="flex items-center gap-3">
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
              {activities.length === 0 ? (
                ...
                activities.slice(0, 8).map((act) => (
                  <div key={act.id} className="p-3 rounded-xl bg-[#fdfafb] border border-[#BD5579]/15 text-xs">
                    ...
                    <div className="text-[10px] text-black font-bold mt-1">{formatTimeAgo(act.timestamp)}</div>
                  </div>
                ))
              )}
            </div>
        ...
        <div className="bg-white border border-[#BD5579]/20 rounded-2xl overflow-hidden shadow-card-subtle">
          <table className="w-full text-left text-xs text-black">
            ...
            <tbody className="divide-y divide-[#BD5579]/10">
              {customers.map((c) => (
                <tr key={c.id} className="hover:bg-[#fcf2f6]/60 transition-colors">
                  ...
                  <td className="px-5 py-4 text-right">
                    <Link
                      to={`/customers/${c.id}`}
                      className="text-xs text-black hover:text-[#601D49] font-bold"
                    >
                      View Profile &rarr;
                    </Link>
                  </td>
          ...
          <table className="w-full text-left text-xs text-black">
            ...
            <tbody className="divide-y divide-[#BD5579]/10">
              {appointments.map((a) => {
                ...
                return (
                  <tr key={a.id} className="hover:bg-[#fcf2f6]/60 transition-colors">
      ...
      {activeTab === 'bills' && (
        <div className="bg-white border border-[#BD5579]/20 rounded-2xl overflow-hidden shadow-card-subtle">
          <table className="w-full text-left text-xs text-black">
            ...
            <tbody className="divide-y divide-[#BD5579]/10">
              {bills.map((b) => (
                <tr key={b.id} className="hover:bg-[#fcf2f6]/60 transition-colors">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {staff.map((s) => (
            <div key={s.id} className="bg-white border border-[#BD5579]/20 rounded-2xl p-5 shadow-card-subtle space-y-3">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-xl bg-[#601D49] text-[#FFEBB8] border-2 border-[#FFEBB8] flex items-center justify-center font-bold text-sm shadow-2xs">
                  {s.name.charAt(0)}
                </div>
                <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {services.map((svc) => (
            <div key={svc.id} className="bg-white border border-[#BD5579]/20 rounded-2xl p-5 shadow-card-subtle flex justify-between items-start">
              <div>
                <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-[#FFEBB8] text-black uppercase tracking-wider border border-[#BD5579]/30">
                  {svc.category}
                </span>
              ...
              <div className="text-right">
                ...
                <div className="text-[10px] text-black font-bold">{svc.duration_minutes}m</div>
              </div>
            </div>
          ))}
        </div>
